...
> [!IMPORTANT]
> **部署后，Sub2API 管理员必须为所有销售分组创建并启用分组 API 密钥。**
>
> 添加中转站并完成首次同步后，请由管理员登录对应的 Sub2API 后台，进入 `/keys`，为每一个销售分组创建并绑定 API 密钥。GatewayOps 需要这些分组密钥读取当前可用模型，并以该分组身份执行真实请求。这与添加中转站时填写的管理 API Key 是两项不同的配置，缺一不可。
>
> **模型价格页面和分组快速测试都依赖这些分组 API 密钥。** 密钥缺失时，模型价格可能缺少模型、显示“价格不完整”或更新失败，分组快速测试会返回 `group_admin_api_key_missing`（HTTP 422）。新建分组、调整分组模型或删除密钥后，也应同步维护 `/keys` 并在 GatewayOps 中执行一次实时同步。未绑定分组的全局 API 密钥可以作为兜底，但建议为每个分组分别创建密钥，避免权限或模型范围不明确。
...
页面展示公开分组的近期调用结果、响应耗时、倍率和可用性趋势。浏览器会先展示上次缓存的数据，再立即在后台更新，并继续每 30 秒自动刷新。状态来自最近调用记录，仅供趋势参考，不代表实时可用性。

### 公开模型价格

每个 Sub2API 中转站都可以分享无需后台登录的模型价格页面：

```text
/public/model-pricing/<中转站 ID>
```

模型价格页只展示该中转站销售分组当前实际可用的模型，并提供：

- 按平台归类并展示真实品牌图标，海外模型平台优先、国产模型平台随后。
- 区分系统默认定价、渠道覆盖定价和分组独立生图定价。
- Token 模型统一按每 1M tokens 展示；按次模型按单次请求展示，生图价格可区分 `1K`、`2K` 和 `4K`。
- 支持按模型、平台、Provider 或渠道关键词搜索，并按平台和计费模式筛选。
- 浏览器缓存价格数据一小时；缓存有效时直接展示，到期后后台更新，也可手动刷新。

模型列表和分组归属来自分组 API 密钥。请先按照 README 顶部的必做提示，为所有销售分组创建并启用密钥，否则页面内容可能不完整。
...
| 公开模型价格 | `/public/model-pricing/:stationID` | 分享中转站当前可用模型及有效价格 |
...
7. 由管理员登录对应 Sub2API 的 `/keys`，为所有销售分组创建并绑定 API 密钥；返回 GatewayOps 再执行一次实时同步。
8. 打开分组快速测试和公开模型价格页，确认分组模型、Token 价格及按次价格均完整。
9. 检查成本倍率和风险建议后，再决定是否开启自动调组或自动优先级。
...
- 公开模型价格页面只展示模型与价格信息，不暴露中转站管理员密钥或分组 API 密钥。
...
- 模型价格
![GatewayOps 模型价格](./docs/images/PixPin_2026-08-29_01-57-27.png)
